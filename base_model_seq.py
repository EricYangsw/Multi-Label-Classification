...
class BaseModel(object):
    def __init__(self, config):
        ...
        self.image_shape = [config.batch_size, 
                            config.time_step, 
                            config.fearute_size] # input shape
    ...
    def evals(self, sess):
        ...
        for i in tqdm(list(range(make_data.num_eval_batches)), desc='batch'):
            ...
            if (c/t) <= 0.008:
                images = np.resize(images, (-1, config.fearute_size)).astype(np.float16)
                idx_output = np.zeros((images.shape[0], config.max_class_label_length), dtype=np.float16)
                print('images.shape[0]:   ', images.shape[0])
                for le in range(config.batch_size-1):
                    raw = config.time_step*le + config.time_step-1
                    idx_output[raw,:] = final_result_max_idx[:, le].astype(np.float16)
                        
                np.savetxt("./val_results/input_maxidx" + str(i) + ".csv", 
                           np.hstack((images, idx_output)), 
                           delimiter=',',
                           fmt='%s')
                np.savetxt("./val_results/final_result_max_idx_" + str(i) + ".csv", 
                           final_result_max_idx, 
                           delimiter=',',
                           fmt='%s')
                np.savetxt("./val_results/final_result_max_value_" + str(i) + ".csv", 
                           final_result_max_value, 
                           delimiter=',',
                           fmt='%s')
